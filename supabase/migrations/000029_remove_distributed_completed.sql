-- Remove distributed and completed stages; patient_served is terminal
...
-- 1. Migrate data
update donation_checks set status = 'blood_recorded' where status::text = 'distributed';
update donation_checks set status = 'patient_served' where status::text = 'completed';
delete from check_events where event_type::text in ('distributed', 'completed');
...
-- 6. Materialized view and integrity report
